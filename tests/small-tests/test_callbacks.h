...
typedef struct sock_ctx {
  std::function<kch_sock_code_t(void* socket_context, const char* host, unsigned int port)> on_connect;
  std::function<kch_sock_code_t(void* socket_context, const char* buffer, size_t length)> on_send;
  std::function<kch_sock_code_t(void* socket_context, char* buffer, size_t length_to_read, size_t* out_actual_length)> on_recv;
  std::function<kch_sock_code_t(void* socket_context)> on_close;
} sock_ctx;
...
inline kch_sock_code_t cb_connect(void* socket_context, const char* host, unsigned int port) {
  sock_ctx* ctx = (sock_ctx*)socket_context;
  return ctx->on_connect(socket_context, host, port);
}

inline kch_sock_code_t cb_send (void* socket_context, const char* buffer, size_t length) {
  sock_ctx* ctx = (sock_ctx*)socket_context;
  return ctx->on_send(socket_context, buffer, length);
}

inline kch_sock_code_t cb_recv(void* socket_context, char* buffer, size_t length_to_read, size_t* out_actual_length) {
  sock_ctx* ctx = (sock_ctx*)socket_context;
  return ctx->on_recv(socket_context, buffer, length_to_read, out_actual_length);
}

inline kch_sock_code_t cb_close(void* socket_context) {
  sock_ctx* ctx = (sock_ctx*)socket_context;
